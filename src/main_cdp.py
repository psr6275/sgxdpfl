...
from models.Update import LocalUpdateDP, LocalUpdateDPSerial, LocalUpdateCDP
...
if __name__ == '__main__':
    ...
    if args.dataset == 'mnist':
        ...
        dataset_train = datasets.MNIST('../data/examples/mnist/', train=True, download=True, transform=trans_mnist)
        dataset_test = datasets.MNIST('../data/examples/mnist/', train=False, download=True, transform=trans_mnist)
        args.num_channels = 1
        # sample users
        if args.iid:
        ...
        dataset_train = datasets.CIFAR10('../data/examples/cifar', train=True, download=True, transform=trans_cifar_train)
        dataset_test = datasets.CIFAR10('../data/examples/cifar', train=False, download=True, transform=trans_cifar_test)
        if args.iid:
        ...
        dataset_train = datasets.FashionMNIST('../data/examples/fashion-mnist', train=True, download=True,
                                              transform=trans_fashion_mnist)
        dataset_test  = datasets.FashionMNIST('../data/examples/fashion-mnist', train=False, download=True,
                                              transform=trans_fashion_mnist)
        if args.iid:
            dict_users = mnist_iid(dataset_train, args.num_users)
        else:
    ...
    if args.dp_method =='cdp':
        clients = [LocalUpdateCDP(args=args, dataset=dataset_train, idxs=dict_users[i]) for i in range(args.num_users)]
    elif args.serial:
    ...
    for iter in range(args.epochs):
        ...
        if args.dp_method != 'cdp':
            ...
            if args.dp_mechanism in ['Laplace', 'Gaussian']:
                sensitivity = cal_sensitivity(args.lr,args.dp_clip, len(dataset_train))
            elif args.dp_mechanism == 'MA':
                sensitivity = cal_sensitivity_MA(args.lr, args.dp_clip, len(dataset_train))
